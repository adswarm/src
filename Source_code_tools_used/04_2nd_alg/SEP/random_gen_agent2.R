...
#                            gen_nb, center_x, center_y, center_z, center_r
...
for (agent_idx in 1:3){
  ...
  if(agent_idx == 1){
    #                            gen_nb, center_x, center_y, center_z, center_r
    df_agent2 <- fct.make_sphere(100000, 145.0596, 84.01725, 12.10435, 23.75571) # <- raw value from the sim
  }else if(agent_idx == 2){
    # [NOTE]
    #'* NOTE: agent2 is 1st drone *
    #'[NOTE]
    #                            gen_nb, center_x, center_y, center_z, center_r
    df_agent2 <- fct.make_sphere(100000, 144.6478, 86.09045, 11.33051, 16.20963) # <- raw value from the sim
  }else{
    #                            gen_nb, center_x, center_y, center_z, center_r
    df_agent2 <- fct.make_sphere(100000, 144.7474, 82.30505, 11.48442, 24.29293) # <- raw value from the sim    
  }
  ...
  if(agent_idx == 1){
    output_filename <- paste(output_dir,"2nd_drone_agent_1_ran_gen.csv" ,sep="")
    write.csv(df_agent2_trimmed_lb, file = output_filename, row.names = FALSE)   
  }else if(agent_idx == 2){
    # [NOTE]
    #'* NOTE: agent2 is 1st drone *
    #'[NOTE][df_agent2_trimmed_ub]
    output_filename <- paste(output_dir,"1st_drone_agent_2_ran_gen.csv" ,sep="")
    write.csv(df_agent2_trimmed_ub, file = output_filename, row.names = FALSE) 
  }else{
    output_filename <- paste(output_dir,"3rd_drone_agent_3_ran_gen.csv" ,sep="")
    write.csv(df_agent2_trimmed_lb, file = output_filename, row.names = FALSE) 
  }
