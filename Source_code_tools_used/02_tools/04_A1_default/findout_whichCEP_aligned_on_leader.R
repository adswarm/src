...
plot(data_raw$l.x, data_raw$l.y, col = 'red', pch = 17, xlim = c(-1.5, 0.5), ylim = c(-1.0, 1.0))
...
points(data_raw$o.x, data_raw$o.y, col = 'gray', pch = 17)
...
for(mode.which in c('f1','f2','f3','o')){ # l is elicited because it is 0, 0
  # for(range_idx in 1:130) { #0:8){
  for(range_idx in 18:18) { #0:8){
    ...
    }else if(mode.which == 'o'){
      data_x <- data_raw$o.x[range_start:range_end]
      data_y <- data_raw$o.y[range_start:range_end]
    }
