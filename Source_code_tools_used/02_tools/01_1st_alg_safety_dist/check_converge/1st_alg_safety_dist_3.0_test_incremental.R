...
output_directory = paste("~/Research/tool_project_swarm_safety/Preprocessing_R/1st_alg_safety_dist_3.0/check_converge/output/",sep="" )
...
datalist <- list.files(path = "~/Research/tool_project_swarm_safety/Preprocessing_R/1st_alg_safety_dist_3.0/check_converge/input")
...
csvname = paste("~/Research/tool_project_swarm_safety/Preprocessing_R/1st_alg_safety_dist_3.0/check_converge/input/",filename,sep="" )
...
plot(data_raw$l.x, data_raw$l.y, col = 'red', pch = 17, xlim = c(-2.0, 1.0), ylim = c(-2.5, -1.0))
...
for(mode.which in c('l','f1','f2','f3')){
  ...
  for(range_idx in 0:8) { #0:8){
    ...
    if(param.mode.savePlot == TRUE){
      ...
      (cep <- getCEP(df2, CEPlevel=0.9, accuracy=FALSE,
