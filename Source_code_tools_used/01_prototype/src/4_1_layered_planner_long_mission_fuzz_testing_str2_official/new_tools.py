...
def contribution_leader(robots, simulation_tick, params, OBSTACLES, target_obs):
    ...
    temp_dcc_sum = temp_leader_global_sp + temp_leader_obs_btt + temp_leader_obs_dia + temp_leader_obs_ltr + temp_leader_wall
...
def put_into_seed_pool(robots, arg_target_index, params, seed_pool_idx):
    ...
    logging.debug("len(robots[target_index].influence_max): [%i], robot[%i]" % (len(robots[target_index].influence_max), arg_target_index))
    for tick_index in range(len(robots[target_index].influence_max)):
...
def make_perturbation(current_spawning_point, mode):
    ...
    if mode == 'big':
        ...
        perturbed_coord[0] = (REGEN_BOUNDARY * rnd_length) * math.cos(rnd_theta)
        perturbed_coord[1] = (REGEN_BOUNDARY * rnd_length) * math.sin(rnd_theta)
...
def random_attack_target_gen(params):
    ...
    logging.debug("ATTACK_TARGET is : [%f][%i]" % (temp_random, params.attack_target))