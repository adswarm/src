...
def draw_map(obstacles):
    ...
    # Bounds on world
    world_bounds_x = [-MAP_BOUNDS_METER, MAP_BOUNDS_METER]
    world_bounds_y = [-MAP_BOUNDS_METER, MAP_BOUNDS_METER]
...
def draw_gradient(f, nrows=MAP_SIZE, ncols=MAP_SIZE):

    skip = 10
    [x_m, y_m] = np.meshgrid(np.linspace(-MAP_BOUNDS_METER, MAP_BOUNDS_METER, ncols),
                             np.linspace(-MAP_BOUNDS_METER, MAP_BOUNDS_METER, nrows))
    [gy, gx] = np.gradient(-f)
...
def postprocessing(robots, metrics, params, visualize=1):
    ...
    if visualize:
        ...
        for robot in metrics.robots:
            plt.plot(robot.route[:, 0], robot.route[:, 1],
                     '--', label='drone %d' % robot.id, linewidth=2)
        plt.plot(metrics.centroid_path[:, 0], metrics.centroid_path[:,
                                                                    1], linewidth=3, label='centroid', color='k')
        plt.legend()
        plt.grid()

    print("\n")
    for robot in metrics.robots:
        print("Robot %d Average Velocity: %.2f [m/s]" %
              (robot.id, np.mean(np.array(robot.vel_array))))
        print("Robot %d Max Velocity: %.2f [m/s]" %
              (robot.id, np.max(np.array(robot.vel_array))))
    ...
    if visualize:
        ...
        plt.plot(metrics.t_array, metrics.robots[0].vel_array,
                 '-', color='k', label='drone 1', linewidth=3)
        for r in range(1, len(metrics.robots)):
            plt.plot(
                metrics.t_array, metrics.robots[r].vel_array, '--', label='drone %d' % (r+1), linewidth=2)
        plt.xlabel('Time, [s]')
    ...
    for i in range(len(metrics.robots[0].route)-1):
        ...
        robots_poses = []
        for robot in metrics.robots:
            robots_poses.append(robot.route[i, :])
        robots_poses.sort(key=lambda p: atan2(
            p[1]-metrics.centroid_path[i, 1], p[0]-metrics.centroid_path[i, 0]))
    ...
    if visualize:
        ...
        plt.plot(metrics.t_array[:-1], metrics.area_array,
                 'k', label='Formation area', linewidth=2)
        plt.plot(metrics.t_array, S0*np.ones_like(metrics.t_array),
                 '--', label='Default area', linewidth=2)
        plt.plot(metrics.t_array, metrics.S_mean*np.ones_like(metrics.t_array),
                 '--', color='r', label='Mean area', linewidth=2)
        plt.xlabel('Time, [s]')
    ...
    if visualize:
        ...
        plt.plot(metrics.t_array, l*np.ones_like(metrics.t_array),
                 '--', label='Default distance', linewidth=2)
        plt.plot(metrics.t_array, metrics.mean_dists_array,
                 label='Mean inter-robots distance', color='r', linewidth=2)
        plt.plot(metrics.t_array, metrics.max_dists_array,
                 label='Max inter-robots distance', color='k', linewidth=2)
        plt.grid()
        plt.xlabel('Time, [s]')
        ...
        surf = ax.plot_surface(X, Y, Z/np.max(Z)*200, cmap=plt.cm.jet,
                               linewidth=0.01, vmin=0.0, vmax=200, zorder=1)
        fig.colorbar(surf, shrink=0.5, aspect=5)
        traj = meters2grid(metrics.centroid_path)
        ax.plot(traj[:, 0], traj[:, 1], 100*np.ones(traj.shape[0]),
                label='Centroid trajectory', linewidth=3, color='r', zorder=0)
        for robot in metrics.robots:
            traj = meters2grid(robot.route)
            ax.plot(traj[:, 0], traj[:, 1], 100*np.ones(traj.shape[0]), '--',
                    label="Robots' trajectories", linewidth=1, color='y', zorder=1)
...
def save_data(robots, metrics, folder_name='output_%f' % time.time()):
