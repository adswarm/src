...
"""
USAGE:
    $ src/layered_planner/planner.py test -k normalize
    $ src/layered_planner/planner.py test -k replay

DEBUG:
    # normal debug
    $ DEBUG=1 src/layered_planner/planner.py test -k normal

    # use test-case map
    $ TESTCASE=1 DEBUG=1 src/layered_planner/planner.py test -k normal
    $ TESTCASE=1 src/layered_planner/planner.py test -k validate
    $ TESTCASE=1 src/layered_planner/planner.py test -k validate -v yes
"""
...
# this is for backup: previous version (DEC)
"""
def move_obstacles(obs, params, special_target, direction_vector):
    temp_target = copy.deepcopy(special_target)
    temp_size = 0.2  # Because the size of direction_vector is 1 (unit vector).

    temp_target[0] = temp_target[0] + temp_size * direction_vector[0]
    temp_target[1] = temp_target[1] + temp_size * direction_vector[1]

    temp_bit = 0.1  # for the size of obstacle: default

    obs[-3] = [[temp_target[0], temp_target[1]], [temp_target[0] + temp_bit, temp_target[1]],
               [temp_target[0] + temp_bit, temp_target[1] + temp_bit], [temp_target[0], temp_target[1] + temp_bit]]
    obs[-2] += np.array([-0.007, 0.0]) * params.drone_vel / 2
    obs[-1] += np.array([0.0, 0.005]) * params.drone_vel / 2
    return obs
"""
...
# should use with TESTCASE env variable


def _import_testcase(args, in_pn):
    """
    generate global plans and store them to PNG files
    ...
    files = glob.glob(in_pn + "/*.pkl")
    for filename in files:

        OBSTACLES = pickle.load(open(filename, 'rb'))
        case_counter += 1
        ...
        out_pkl = os.path.join(TESTCASE_PN, "case%d.pkl" % case_counter)
        out_png = os.path.join(TESTCASE_PN, "case%d.png" % case_counter)

        draw_map(OBSTACLES)  # for background environment
        plt.plot(
            xy_start[0], xy_start[1], 'bo', color='red',
        ...
        shutil.copyfile(filename, out_pkl)


def fuzz_one(args, obstacle_type, check_crash=False, manual_mode=False, param_setting='None', randometest_order=1):
    ...
    if param_setting is 'tester':
        print("[tester]fuzz_one starts")
        # TODO: check it's necessary or not.

    elif param_setting is 'normal':
        print("[normal]fuzz_one starts")
        # TODO: this mode will be deprecated
        # when normal, it should be recorded and yield 'pure' npy.
        # so it is not needed.

    elif param_setting is 'findcscore':
        print("[findcscore]fuzz_one starts")

        params.contribution = True

        # Off the rest setting
        params.mode_replay = False
        ...
        params.mode_record_trajectory_replay = False  # no need to record npy files
        params.already_written_allsp = False
        params.mode_defective = False

    elif param_setting is 'record':
    ...
    elif param_setting is 'replay':
        print("[replay]fuzz_one starts")
        # TODO: this mode can be merged into record.
        # when normal, it should be recorded and yield 'pure' npy.
        # when replay, it should be recorded and yield 'rollback ver' npy

    elif param_setting is 'randomtest':
        print("[randomtesting]fuzz_one starts")
        params.mode_replay = True  # make sure it loads info from npy from partial replay
        # fixed, if it is set as True, it will be turned off as soon as it recog.
        params.mode_stop_before_obs = False
        params.crash_check_for_random_testing = True
        params.mode_randomtesting = True
        # if mode_randomtesting is True, sim loads coordinates from not 'pure' but 'rollback ver' folder
        # insertion
        # in this case, make sure it writes only coord[-1].
        params.mode_record_trajectory_replay = False  # True, either is ok
        # for faster sim, set mode_record_trajectory_replay False.
        params.already_written_allsp = False  # when it is False, it outputs all_sp
        params.mode_defective = False
        # make sure sim is not ended after recording coordinates on all_sp.log

    elif param_setting is 'validate':
        print("[validate]fuzz_one starts")
        # TODO:
    else:
...
simul_tick [%d] with [%f] in zone_[%d] speedX [%f] obsSize [%f]" % \
    ...
    if params.record_contribution_score is True:
        ...
        # end_seed_pool_idx = params.end_seed_pool_idx  # <- param
        ...
        for seed_pool_idx in range(start_seed_pool_idx, end_seed_pool_idx):
            ...
            temp_temp_dist_for_fuzz = 1.0/3.0 * (distance_between_dcc(ref_f1, robots[1].dcc) + distance_between_dcc(
                ref_f2, robots[2].dcc) + distance_between_dcc(ref_f3, robots[3].dcc))

            if temp_temp_dist_for_fuzz <= min_dcc:
                min_dcc = temp_temp_dist_for_fuzz
                min_dcc_seed = seed_pool_idx

            # temp_dist_for_fuzz += temp_temp_dist_for_fuzz
            print("1-1. Reading files from: " +
                  SEEDPOOL_DIR + "/"+str(seed_pool_idx) + ", and dist: "+str(temp_temp_dist_for_fuzz))

        # final_dist_for_this_run = temp_dist_for_fuzz / end_seed_pool_idx

        # print("2. Compared to ["+str(end_seed_pool_idx) + "], Final distance of current run: " +
        #       str(final_dist_for_this_run))

        if min_dcc <= params.fuzz_threshold:  # configured by user
        ...
        # if params.flag_meaningless == False:
            ...
            put_into_seed_pool(robots, arg_target_index=3,
                               params=params, seed_pool_idx=end_seed_pool_idx)
            # seed pool change: param.end_seed_pool_idx += 1
            # params.end_seed_pool_idx += 1
            update_size_pool(SEEDPOOL_DIR + "/poolsize.csv",
                             end_seed_pool_idx + 1)

            # purturb small #<- param.purturb standard
...
def long_exp():
    print("record_trajectories is starting")
    ...
    starting_point = np.array([1.2, 1.25])  # [-1.2, 1.25] when 1.0
    ...
    # print("des2: "+str(des2) + " / des3: " +
    #       str(des3) + " / des4: " + str(des4))

    # for index_row in range(len(referFile)):
    for seed_idx in range(1, 1001):  # 3001

        # p_target_set = [1,2,3] # fixed in this mode 1 means f1, 2 means f2, 3 means f3
        p_target = 1
        p_seed = seed_idx
        p_coef = None
        p_purpose = None  # for loop # can be one of [b, a, r, i, v, d]
        # 0829, for now do 'r' sim
        p_idx = None
        p_tick = None

        '''robots_starting_point'''

        # when it is set as [1.0], it yields 'pure' trajectory
        # variation_p_coef = [1.0]
        # variation_p_idx = [160]
        ...
        # elif seed_idx == 2:
        #     obstacle_set = obstacle_set_2
        # elif seed_idx == 3:
        #     obstacle_set = obstacle_set_3
        # elif seed_idx == 4:
        #     obstacle_set = obstacle_set_4
            ...
            for y in variation_p_idx:
                for z in variation_p_purpose:
                    for obstacle_type in obstacle_set:
                        ...
                        print("##################################################")
                        print(str(seed_idx)+"th randometesting.................. ")
                        print(
                            "[Exp103-str4-vel] Strategy 1 with various vel")
                        print("##################################################")

                        # 1st trial
                        fuzz_one(args, obstacle_type, param_setting='record')


def record_trajectories_pure():
    print("record_trajectories is starting")
    ...
    for index_row in range(len(referFile)):
        ...
        p_target = 2  # fixed in this mode
        p_seed = referFile[index_row][index_seed]
        p_coef = None
        p_purpose = None  # for loop # can be one of [b, a, r, i, v, d]
        # 0829, for now do 'r' sim
        p_idx = None
        ...
        variation_p_coef = [0.2, 2.0]
        # variation_p_idx = [20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240]
        variation_p_idx = [0]  # for pure recording
        variation_p_purpose = ['r', 'a', 'i', 'b']
        ...
        for x in variation_p_coef:
            for y in variation_p_idx:
                for z in variation_p_purpose:
                    ...
                    p_purpose = z
                    args = [p_target, p_seed, p_coef, p_purpose, p_tick, p_idx]
                    fuzz_one(robot1, args, param_setting='record')


def randomtest():

    # referfilename = 'randomtesting/pure_recog_time.csv'
    # referFile = np.array(pd.read_csv(referfilename, sep=','))

    # CONTROL THIS PART
    # input_file_name = ['r_x0.2','r_x2.0','i_x0.2','i_x2.0','b_x0.2','b_x2.0','a_x0.2','a_x2.0']
    input_file_name = ['i_x2.0']  # , 'r_x2.0']
    # p_set = [140, 160, 140, 160, 160, 100, 220, 100]
    p_set = [0]  # , 160]
    ...
    for i in range(0, 1):
        ...
        # CONTROL THIS PART
        filename = 'randomtesting/input/1000/randomized_' + \
            str(input_file_name[i])+'_rollback_'+str(p_idx)+'.csv'
        inputFile = np.array(pd.read_csv(filename, sep=','))

        print("[log]argument info is from: "+filename)

        index_seed = 1  # 24 fixed
        index_coef = 3  # 26 fixed
        index_purpose = 2  # 'r' fixed
        index_tick = 5  # 25 fixed

        # each filename has about 65 row, which is equivalent to #_run
        for index_row in range(575, len(inputFile)):
            # temp_set = [1, 2, 3, 4]
            # temp_set = [3104,3104,3104,3104]#1, 441, 2481, 3919]#[3919]#
            # temp_set = [4136, 4916]
            # for index_row in temp_set:

            p_target = 2        # fixed, not used in this mode
            # read from filename corresponding col
            p_seed = int(inputFile[index_row][index_seed])
            # read from filename corresponding col
            p_coef = int(inputFile[index_row][index_coef])
            # read from filename corresponding col
            p_purpose = inputFile[index_row][index_purpose]
            # can be one of [b, a, r, i, v, d]

            p_tick = int(inputFile[index_row][index_tick])
            # read from referFile 2nd col = [1]
            # Note: exact time that new coor. is inserted is 'recog.t - 1'

            args = [p_target, p_seed, p_coef, p_purpose, p_tick, p_idx]
            print("  ")
            print(str(index_row)+"th randometesting.................. ")
            fuzz_one(args, param_setting='randomtest',
                     randometest_order=index_row)


"""
this is only for plotting or post-processing
no need to care.
"""
arg_01 = 0
metrics = Metrics()
...
if __name__ == '__main__':
    ...
    tester = subparser.add_parser(
        'test', help='Tester', add_help=False
    )
    tester.add_argument(
    ...
    tester.add_argument(
        "-v", "--validate", dest="validate", type=str,
        default=None, help="Dry-run all cases", required=False
    )
    tester.set_defaults(action='tester')
    ...
    if args.action == "tester":
        ...
        elif kind == "randomtest":
            randomtest()

        # elif kind == "findcscore":
        #     findcscore()
