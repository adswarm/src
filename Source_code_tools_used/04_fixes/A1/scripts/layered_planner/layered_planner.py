...
class Params:
    def __init__(self):
        ...
        # show constructed paths at the end of the RRT and path smoothing algorithms
        self.visualize = 1
        ...
        # [m], extension parameter: this controls how far the RRT extends in each step.
        self.extension = 0.4
        ...
        # [m], maximum distance between current robot's pose and the sp from global planner
        self.max_sp_dist = 0.15 * self.drone_vel
        # potential fields radius, defining repulsive area size near the obstacle
        self.influence_radius = 0.15
        """fix
        self.influence_radius = 0.30
        """
        ...
        # [m], size of Vicon objects: moving cubes, other drones
        self.length_moving_obstacles = 0.2
        self.reached_goal = 0
        # [m], distance between the drones in the formation
        self.l_drones = 0.3
...
class Robot(Drone):
    ...
    def local_planner(self, obstacles, params):
        ...
        self.f = combined_potential(
            obstacles_grid, self.sp_global_planner[:2], params.influence_radius)
...
obstacles = [
    # narrow passage
    np.array([[-2.5, -0.5], [-passage_location-passage_width/2., -0.5],
             [-passage_location-passage_width/2., 0.5], [-2.5, 0.5]]),
...
if __name__ == '__main__':
    ...
    draw_map(obstacles)
    plt.plot(xy_start[0], xy_start[1], 'bo',
             color='red', markersize=20, label='start')
    plt.plot(xy_goal[0], xy_goal[1], 'bo',
             color='green', markersize=20, label='goal')
    ...
    plt.plot(P[:, 0], P[:, 1], linewidth=3,
             color='orange', label='Global planner path')
    plt.pause(0.1)

    sp_ind = 0
    robot1.route = np.array(
        [[traj_global[0, 0], traj_global[0, 1], params.TakeoffHeight]])
    robot1.sp = robot1.route[-1, :]

    followers_sp = formation(
        params.num_robots, leader_des=robot1.sp[:2], v=np.array([1, 0]), l=params.l_drones)
    ...
    if params.toFly:
        ...
        for t in range(3):
            ...
            for cf in cf_list:
                cf.takeoff(targetHeight=params.TakeoffHeight,
                           duration=4*params.TakeoffHeight)
    ...
    while not rospy.is_shutdown():  # loop through all the setpoint from global planner trajectory, traj_global
        ...
        # [m]
        if dist_to_goal < params.goal_tolerance and not params.reached_goal:
        ...
        for i in range(len(moving_obstacles)):
            obstacles[moving_obstacles_start_index +
                      i] = pose2square(moving_obstacles[i].position()[:2], l)

        # leader's setpoint from global planner
        robot1.sp_global_planner = np.array(
            traj_global[sp_ind, :].tolist() + [params.TakeoffHeight])
        # correct leader's pose with local planner

        robot1.local_planner(obstacles, params)

        """fix
        use below instead of 239~248

        robots_obstacles_sp = [x for i, x in enumerate(
            followers_sp + [robot1.sp[:2]]) if i != p]  # all poses except the robot[p]
        ...
        obstacles1 = obstacles + robots_obstacles
            # follower robot's position correction with local planner
        
        robot1.local_planner(obstacles1, params)
        ...
        followers_sp_global_planner = formation(
            params.num_robots, robot1.sp_global_planner[:2], v=direction, l=params.l_drones)
        for i in range(len(followers_sp_global_planner)):
            robots[i+1].sp_global_planner = followers_sp_global_planner[i].tolist() + \
                [params.TakeoffHeight]
        ...
        for robot in robots:
            plt.plot(robot.sp[0], robot.sp[1], '^', color='blue',
        ...
        for robot in robots[1:]:
            plt.plot(robot.route[:, 0], robot.route[:, 1],
                     '--', linewidth=2, color='green', zorder=10)
        plt.plot(P[:, 0], P[:, 1], linewidth=3,
                 color='orange', label='Global planner path')
        plt.plot(traj_global[sp_ind, 0], traj_global[sp_ind, 1], 'ro',
        ...
        if sp_ind < traj_global.shape[0]-1 and norm(robot1.sp_global_planner - robot1.sp) < params.max_sp_dist:
            sp_ind += 1
        """ fix 
        ...
        # if condition_1 and condition_2 : sp_ind += 1
        """
