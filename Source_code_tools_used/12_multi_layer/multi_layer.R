...
param.mode.search <- TRUE

param.dist_criteria <- 0.02
param.neighborNum_criteria <- 2
param.required_subset_size_rate <- 1.0
param.delta <- 0.1
...
for(i in 1: endLoop){   #length(datalist)){
  ...
  param.dist_criteria <- 0.02
  param.neighborNum_criteria <- 2
  param.required_subset_size_rate <- param.required_subset_size_rate
  param.delta <- 0.1
  ...
  token_minus <- FALSE
  j <- 0
  if(param.mode.search == TRUE){
    
    repeat{
      j <- j + 1
      cat("token[",j,"]: ",token_plus, "_", token_minus, "\n")
      ...
      if(standard == 'f1'){
        ...
        cat('stadard is f2')
        current_subset_size_rate <- nrow(f2_sp_after_prunned) / nrow(f2_sp) #standard is f2  
      }else if(standard == 'f3'){
        cat('stadard is f3')
        current_subset_size_rate <- nrow(f3_sp_after_prunned) / nrow(f3_sp) #standard is f3  
      }
    ...
    current_subset_size_rate <- 1.0
    
    f1_sp_after_prunned_mean <- c(mean(f1_sp[[1]]), mean(f1_sp[[2]]))
    f2_sp_after_prunned_mean <- c(mean(f2_sp[[1]]), mean(f2_sp[[2]]))
    f3_sp_after_prunned_mean <- c(mean(f3_sp[[1]]), mean(f3_sp[[2]]))
    
    f1_sp_after_prunned_var <- c(var(f1_sp[[1]]), var(f1_sp[[2]]))
    f2_sp_after_prunned_var <- c(var(f2_sp[[1]]), var(f2_sp[[2]]))
    f3_sp_after_prunned_var <- c(var(f3_sp[[1]]), var(f3_sp[[2]]))
  ...
  result <- paste("[log] ",filename," ",param.dist_criteria ," ", current_subset_size_rate,
                  " subset's mean: ",f1_sp_after_prunned_mean," ",
                  f2_sp_after_prunned_mean," ",
                  f3_sp_after_prunned_mean," ",
                  " var: ",f1_sp_after_prunned_var," ",
                  f2_sp_after_prunned_var," ",
                  f3_sp_after_prunned_var,sep="")
  
  write.table(result, "~/Research/tool_project_swarm_safety/Preprocessing_R/data/02_01_output_plot/result_multi_layer.csv", 
              append = TRUE, sep = " ", col.name = FALSE, row.names = FALSE, quote = FALSE)
  ...
  # 
  # plot(c(f1_sp[,1],f2_sp[,1],f3_sp[,1]),c(f1_sp[,2],f2_sp[,2],f3_sp[,2]), pch = 2, xlab="x", ylab="y", main = plotname)
  # points(f1_sp, col = "blue", pch = 2)
  # points(f2_sp, col = "brown", pch = 2)
  # points(f3_sp, col = "orange", pch = 2)
  # 
  # 
